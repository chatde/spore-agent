...
function codeScore(s: string): number {
  let sc = 0;
  if (s.includes('function') || s.includes('=>')) sc += 20;
  if (s.includes('return')) sc += 15;
  if (s.includes('{')) sc += 10;
  if (s.length > 20) sc += 15;
  if (s.length > 100) sc += 10;
  return clamp(sc + rand(5, 20));
}
function reasonScore(s: string): number {
  const m = ['therefore', 'because', 'since', 'thus', 'hence', 'if', 'then', 'given', 'conclude', 'follows', 'implies'];
  let sc = has(s, m) * 7;
  if (wc(s) > 30) sc += 15;
  if (wc(s) > 80) sc += 10;
  return clamp(sc + rand(5, 20));
...
function creativeScore(s: string): number {
  const u = new Set(s.toLowerCase().split(/\s+/));
  let sc = Math.min(40, u.size);
  if (wc(s) > 20) sc += 15;
  return clamp(sc + rand(5, 20));
}
...
function mathScore(s: string): number {
  let sc = 0;
  if (/\d/.test(s)) sc += 20;
  if (s.includes('=') || s.includes('+')) sc += 15;
...
export const P25_EXT: Record<string, GameEngine> = {
  ...
  p25_game_9: textGame({
    ...
    score: (answer, d) => {
      let sc = 0;
      sc += has(answer.toLowerCase(), ['rto', 'failover', 'post-mortem']) * 30;
      sc += precisionScore(answer, 'database cluster failover experiment') * 40;
      sc += creativeScore(answer) * 30;
      return clamp(sc * (1 + d / 20));
    },
    deadline: 140,
