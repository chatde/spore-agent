...
export const P32_EXT: Record<string, GameEngine> = {
  ...
  'metadata_preservation_02': textGame({
    prompts: [
      (d, r) => `You've inherited a digital archive of scanned historical photographs. Each photo has metadata including: filename, scan date, scanner model, resolution, color depth, original physical size. What key considerations for ensuring the authenticity and integrity of this metadata?`,
  ...
  'metadata_preservation_03': textGame({
    prompts: [
      (d, r) => `You are archiving a large collection of video game ROMs. Each ROM has the following metadata: ROM Filename, Game Title, Developer, Publisher, Release Year, Genre, Region. How would you handle potential conflicts or inconsistencies in this metadata?`,
      (d, r) => `You are archiving a collection of born-digital documents (Word, Excel, PDFs) created by a government agency.  Metadata includes: author, creation date, modification date, file size, file type. What metadata schema would you use to represent this information, and why?`,
      (d, r) => `You are archiving a collection of born-digital art (images, videos, interactive installations).  Metadata includes: title, artist, creation date, file format, file size, software dependencies. What strategies would you use to address the issue of bit rot and data corruption?`,
    ],
    score: (answer, d) => reasonScore(answer),
    deadline: 120,
  }),
  'metadata_preservation_04': textGame({
    prompts: [
      (d, r) => `You've inherited a digital archive of scanned historical photographs. Each photo has the following metadata: Filename, Date Taken, Camera Model, GPS Coordinates. Explain the importance of using controlled vocabularies and standardized metadata formats.`,
  ...
  'metadata_preservation_05': textGame({
    prompts: [
      ...
      (d, r) => `You are archiving a collection of born-digital documents (Word .doc files) from the 1990s.  The metadata includes: Author, Date Created, Date Modified, Revision Number, File Size. What are the best practices for securing this data against unauthorized access?`,
      (d, r) => `You are tasked with archiving a collection of born-digital art (e.g., interactive installations, net art, digital video).  Metadata includes: title, artist, creation date, file format, file size, software dependencies. What challenges do you anticipate in preserving the meaning and context of this art?`,
    ],
    score: (answer, d) => reasonScore(answer),
    deadline: 120,
  }),
