...
function precisionScore(s: string, ideal: string[]): number { const len = wc(s); if (len === 0) return 0; return clamp(100 - Math.abs(ideal.filter(i => s.includes(i)).length - 1) * 3); }
...
export const P4_EXT: Record<string, GameEngine> = {
  ...
  logic_labyrinth: textGame({
    ...
    score: (answer, d) => {
      let sc = 0;
      sc += codeScore(answer) ? 15 : 0;
      sc += has(answer, ['¬', '∧', '→']) ? 10 : 0;
      sc += reasonScore(answer) ? 20 : 0;
  ...
  proof_prism: textGame({
    ...
    score: (answer, d) => {
      ...
      sc += mathScore(answer) * 2;
      sc += reasonScore(answer) ? 15 : 0;
      sc += creativeScore(answer) ? 10 : 0;
      return clamp(sc);
    },
    deadline: 180,
  }),
  ...
  reason_roulette: textGame({
    ...
    score: (answer, d) => {
      ...
      sc += mathScore(answer) ? 10 : 0;
      sc += reasonScore(answer) ? 25 : 0;
      return clamp(sc);
    },
    deadline: 150,
  }),
  ...
  paradox_parade: textGame({
    ...
    score: (answer, d) => {
      ...
      sc += reasonScore(answer) ? 20 : 0;
      sc += mathScore(answer) ? 15 : 0;
      sc += creativeScore(answer) ? 15 : 0;
      return clamp(sc);
    },
    deadline: 200,
  }),
