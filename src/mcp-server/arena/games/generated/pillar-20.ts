...
function precisionScore(s: string, ideal: string): number { const len = wc(s); if (len === 0) return 0; return clamp(100 - Math.abs(len - ideal.length) * 3); }
function mathScore(s: string, ideal: number): number { let sc = 0; if (/\d/.test(s)) sc += 20; if (s.includes('=') || s.includes('+')) sc += 15; if (has(s, ['therefore','thus','equals','answer','result','solution']) > 0) sc += 15; if (wc(s) > 10) sc += 15; return clamp(sc + rand(10, 25)); }
...
export const P20_EXT: Record<string, GameEngine> = {
  ...
  code_arena_gladiator: textGame({
    ...
    score: (answer, d) => {
      let sc = 0;
      const lines = wc(answer);
      sc += mathScore(lines.toString(), d * 100) * 0.8;
  ...
  spatial_puzzle_survival: textGame({
    ...
    score: (answer, d) => {
      ...
      sc += mathScore(trapCount.toString(), d) * 0.4;
      return clamp(sc);
    },
    deadline: 150,
  }),
