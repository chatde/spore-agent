...
export const P27_EXT: Record<string, GameEngine> = {
  ...
  'constitution_007':
  textGame({
    prompts: [() => `A suspect is arrested and interrogated for several hours without being read their Miranda rights. During the interrogation, they confess to the crime. Later, during a separate, non-custodial interview, they reiterate their confession after being read their rights. Can this confession be used in court?\n\nOptions:\n(A) Yes, because the initial Miranda violation 'tainted' the subsequent confession, making it inadmissible. (B) No, because the second confession was made after a proper Miranda warning in a non-custodial setting, breaking the chain of causation. (C) Yes, if the police can prove the suspect was not coerced into confessing. (D) No, unless the suspect had an attorney present during the second confession.\n\nAnswer with the letter and explain your reasoning.`],
    score: (a, d) => {
      const l = a.toLowerCase();
      let sc = 0;
      if (l.includes('b') || l.includes('"b"') || l.includes("'b'")) sc += 50;
  ...
  'constitution_009':
  textGame({
    prompts: [() => `During a severe economic downturn, the President issues an executive order cancelling all outstanding student loan debt for public university graduates, citing a national emergency. Opponents challenge this as an overreach of executive power.\n\nOptions:\n(A) The executive order is constitutional as the President has broad authority to manage federal programs during emergencies. (B) The executive order is unconstitutional as it exceeds the President's authority and requires Congressional approval. (C) The order is constitutional if Congress fails to act on the student loan issue within a reasonable timeframe. (D) The order is constitutional only if the President declares a formal national emergency beforehand.\n\nAnswer with the letter and explain your reasoning.`],
    score: (a, d) => {
      const l = a.toLowerCase();
      let sc = 0;
      if (l.includes('b') || l.includes('"b"') || l.includes("'b'")) sc += 50;
  ...
  'constitution_014':
  textGame({
    prompts: [() => `A state passes a law that allows for expedited deportation proceedings for non-citizens without a formal hearing or judicial review, if they are deemed a \'national security threat\' by an executive agency. Civil liberties groups challenge this as a violation of due process.\n\nOptions:\n(A) The law is constitutional as the government has broad powers to protect national security. (B) The law is unconstitutional as it violates due process rights and the right to a meaningful opportunity to be heard. (C) The law is constitutional if the executive agency provides some form of internal review. (D) The constitutionality depends on whether the non-citizens are from countries designated as state sponsors of terrorism.\n\nAnswer with the letter and explain your reasoning.`],
    score: (a, d) => {
      const l = a.toLowerCase();
      let sc = 0;
      if (l.includes('b') || l.includes('"b"') || l.includes("'b'")) sc += 50;
  ...
  'constitution_017':
  textGame({
    prompts: [() => `A state passes a law prohibiting the display of any flag or symbol associated with a foreign country on public property. A group wants to display a flag representing their heritage. They challenge this law.\n\nOptions:\n(A) The law is constitutional as it promotes patriotism and national unity. (B) The law is unconstitutional as it restricts freedom of expression and potentially violates the rights of minority groups. (C) The law is constitutional if it applies equally to all foreign flags and symbols. (D) The constitutionality depends on whether the flag represents a country with which the state has diplomatic relations.\n\nAnswer with the letter and explain your reasoning.`],
    score: (a, d) => {
      const l = a.toLowerCase();
      let sc = 0;
      if (l.includes('b') || l.includes('"b"') || l.includes("'b'")) sc += 50;
  ...
  'constitution_019':
  textGame({
    prompts: [() => `A state passes a law requiring all public schools to display the Pledge of Allegiance at the beginning of each school day. Students who object can be required to stand and recite it. Parents challenge this law, arguing it violates their religious freedom.\n\nOptions:\n(A) The law is constitutional as it promotes patriotism and civic education. (B) The law is unconstitutional as it mandates a religious practice and infringes upon the freedom of religion. (C) The law is constitutional if students are allowed to opt-out of reciting the Pledge. (D) The constitutionality depends on whether the state provides an alternative activity for students who object to the Pledge.\n\nAnswer with the letter and explain your reasoning.`],
    score: (a, d) => {
      const l = a.toLowerCase();
      let sc = 0;
      if (l.includes('b') || l.includes('"b"') || l.includes("'b'")) sc += 50;
  ...
  'constitution_028':
  textGame({
    prompts: [() => `A state passes a law requiring all public schools to teach a version of the American Revolution that emphasizes the role of individual liberty and limited government. Parents challenge this law, arguing it promotes a particular political ideology.\n\nOptions:\n(A) The law is constitutional as it promotes civic education and historical understanding. (B) The law is unconstitutional as it promotes a particular political ideology and infringes upon academic freedom. (C) The law is constitutional if the curriculum is neutral and objective. (D) The constitutionality depends
