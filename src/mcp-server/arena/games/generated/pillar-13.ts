...
regression_analysis: textGame({
  ...
  prompts: [
    ...
    (d, r) => `Analyze this dataset and create a linear regression model:
...
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["imputation", "kNN", "mean"])) sc += 30;
    if (has(answer, ["time-series", "autocorrelation"])) sc += 20;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["feature engineering", "mutual information"])) sc += 35;
...
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["bias", "fairness", "mitigation"])) sc += 30;
    if (has(answer, ["adjust correlation", "bias correction"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["redundancy", "fault tolerance"])) sc += 25;
    if (has(answer, ["parallel processing", "runtime optimization"])) sc += 30;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["power analysis", "sample size"])) sc += 35;
    if (has(answer, ["BIC", "GMM", "Bayesian"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["outlier", "visualization", "3D"])) sc += 30;
    if (has(answer, ["Simpson", "paradox", "correction"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["feature selection", "VIF"])) sc += 30;
    if (has(answer, ["collinearity", "variance inflation"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["SMOTE", "class imbalance"])) sc += 30;
    if (has(answer, ["threshold optimization", "AUC", "precision-recall"])) sc += 35;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["PCA", "dimensionality reduction"])) sc += 35;
    if (has(answer, ["regularization", "L1", "L2"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["A/B testing", "confidence interval"])) sc += 30;
    if (has(answer, ["MNAR", "imputation"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
game_id: textGame({
  ...
  score: (answer, d) => {
    let sc = 0;
    if (has(answer, ["dashboard", "interactive visualization"])) sc += 30;
    if (has(answer, ["isolation forest", "anomaly detection"])) sc += 25;
    return clamp(sc);
  },
  deadline: 120,
}),
